...
struct ruby_newsTests {
    ...
    @Test func imageCacheFetchesFromNetworkOnFirstCall() async throws {
        var fetchCount = 0
        let cache = ImageCache(
            cacheDirectory: makeTempCacheDir(),
            fetch: { _ in
                fetchCount += 1
    ...
    @Test func imageCacheReturnsSameImageFromMemoryOnSecondCall() async throws {
        var fetchCount = 0
        let cache = ImageCache(
            cacheDirectory: makeTempCacheDir(),
            fetch: { _ in
    ...
    @Test func imageCacheLoadsFromDiskAfterMemoryCacheCleared() async throws {
        var fetchCount = 0
        let cache = ImageCache(
            cacheDirectory: makeTempCacheDir(),
            fetch: { _ in
    ...
    @Test func imageCacheReturnsNilForBadImageData() async throws {
        let cache = ImageCache(
            cacheDirectory: makeTempCacheDir(),
            fetch: { _ in Data("not-an-image".utf8) }
        )
