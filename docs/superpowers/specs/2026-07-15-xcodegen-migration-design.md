...
- 모든 `XCBuildConfiguration`의 `baseConfigurationReference`가 `Config/Signing.xcconfig`.
  ...
  - 내용: `APPLE_DEVELOPMENT_TEAM` 정의 → `DEVELOPMENT_TEAM = $(APPLE_DEVELOPMENT_TEAM)`.
...
2. **`.gitignore`** — `ruby-news.xcodeproj/` 추가.
...
targets:
  ruby-news:
    ...
    dependencies:
      ...
      - package: SwiftLint          # buildToolPlugin: SwiftLintBuildToolPlugin
    settings:
      base:
...
- **CLANG_WARN 등 표준 경고 설정은 명시하지 않는다.** XcodeGen 기본 `settingPresets`가
  Xcode 신규 프로젝트 기본값을 채운다. non-default 값만 `settings.base`에 기입한다.
