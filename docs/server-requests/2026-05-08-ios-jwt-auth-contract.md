...
| public 뉴스 목록 | `/api/v1/articles`, `/api/v1/articles/tag/:keyword`, `/api/v1/articles/others` 비로그인 접근 가능 |
| 현재 사용자 조회 | `GET /account/edit` JSON |

## iOS 최종 제품 방향

- 로그인은 Native SwiftUI 폼 + JSON 요청으로 처리한다.
- 회원가입/계정 설정/기사 상세/포스트 상세는 기존 Hotwire Native를 유지한다.
...
- Authorization 있음: `200 OK`, 현재 사용자 기준 `liked` / `boosted` 계산
...
부스트도 같은 인증 및 refresh retry 정책을 사용한다.

```http
POST /api/v1/articles/:article_id/boost
DELETE /api/v1/articles/:article_id/boost
Authorization: Bearer <access_token>
```
...
동시에 여러 Native 요청이 401을 반환해도 iOS는 refresh 요청을 한 번만 수행하고
그 결과를 대기 중인 요청들이 공유한다.
